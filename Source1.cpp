		...
		if (dy == 0) {              // персонаж не прыгает
			if (a == 0) {
				...
				if (curFrame > 1 && curFrame < 1.95)
					curFrame = 1.95;
				if (curFrame > 2)  // возвращение (обычные глаза) 
					curFrame -= 2;
			}
...
int main() {
	...
	C[1].loadFromFile("2.png");
	C[2].loadFromFile("3.png");
	C[3].loadFromFile("4.png");
	C[4].loadFromFile("5.png");
	C[5].loadFromFile("6.png");
	for (int i = 0; i < 6; i++) {
	...
	float Fp = 0;                                //cкорость анимации птички
